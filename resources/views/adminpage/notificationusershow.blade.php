                        ...
                        @foreach ($datanotification as $item)
                            <div id="alert-additional-content-{{$item->id}}"
                                ...
                                <div class="flex items-center">
                                    ...
                                    <h3 class="text-lg font-medium">A New Notification from 
                                    <span class="capitalize"> {{ $item['from'] }} </span>    
                                    </h3>
                                </div>
                                <div class="mt-2 mb-4 text-sm">
                                    {{-- <p class="">TITLE :{{ $item['title'] }}</p> --}}
                                    <p class="capitalize">{{ $item['title'] }}</p>
                                    <span class="">{{ \Carbon\Carbon::parse($item['date'])->format('l jS F Y') }}</span>
                                    {{-- <p class="">TO : {{ $item['to'] }}</p> --}}

                                </div>
                                <div class="flex">
                                    <button data-modal-target="viewmoremodal{{$item->id}}" data-modal-toggle="viewmoremodal{{$item->id}}" type="button"
                                    ...
                                    <form action="/notifreadupdate/{{$item->id}}" method="POST">
                                        @csrf
                                            ...
                                            aria-label="Close">
                                            Dismiss </button>
                                            {{-- data-dismiss-target="#alert-additional-content-{{$item->id}}" --}}
                                </form>
                                </div>
                            </div>
                ...
                <div class="p-5">
                    <div class="flex">
                        <div>
                            ...
                            <p class="mb-3 font-normal text-gray-800"><span class="normal-case">{{ $item['description'] }}
                                </span> </p>
                        </div>
                    </div>
                </div>
                ...
                <form action="/notifreadupdate/{{$item->id}}" method="POST">
                    ...
                    <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button type="submit"
                            ...
                            {{-- data-modal-hide="static-modal_{{ $item->id }}" --}}
                    </div>
                </form>
            </div>
        </div>
