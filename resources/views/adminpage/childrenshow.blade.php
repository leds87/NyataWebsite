                        ...
                        @foreach ($data as $item)
                            <tr class="border-b dark:border-neutral-500">
                                ...
                                <td class="whitespace-nowrap px-6 py-4">
                                        ...
                                        <div class="image-container inline-flex mx-4 mb-4">
                                            @foreach ($item->images as $image)
                                                <img src="{{ asset('storage/children-images/' . $image->filename) }}"
                                                    alt="{{ $image->filename }}" class="w-10 h-10 object-cover rounded-md">
                                                <div class="mx-2"></div>
                                            @endforeach
                                        </div>
                                    @else
                                    ...
                                    @endif
                                <td class="whitespace-nowrap px-6 py-4">
                                    <button type="button" class="bg-green-600 text-white px-2 py-2 rounded-lg"> <a
                                            href="/childrenedit/{{ $item->id }}/edit">Edit</a>
                                    </button>
                                    <form action="/childrendestroy/{{ $item->id }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 text-white px-2 py-2 rounded-lg"
